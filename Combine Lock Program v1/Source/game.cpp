...
void init( void )
{
    ...
    // Implemented by denniswong10 (Combine Lock Program Initi)
    program = nullptr;
...
{
    if (Player::CheckOnJewel()) { g_eGameState = S_MiniGame; }
}
...
void render_MiniGame()
{
    ...
    if (program != nullptr)
    {
        std::ifstream ifs("CombineLock.txt", std::ifstream::in);
        ...
        while (ifs.good()) {
            for (int row = 0; row < 9; row++)
            {
                for (int col = 0; col < 18; col++)
                {
                    ...
                    if (c == '0')
                    {
                        g_Console.writeToBuffer(pos, '0' + program->GetOutput_Number(&index));
                        program->StoreNumber(index, program->GetOutput_Number(&index));
                        index++;
                    }
                    else if (c == 'a')
                    {
                        g_Console.writeToBuffer(pos, program->GetOutput_Expression(&index));
                        index++;
                    }
                    else if (c == '?')
                    {
                        g_Console.writeToBuffer(pos, '0' + program->GetOutputFixed(&index2));
                        index2++;
                    }
                    else
                    {
                        g_Console.writeToBuffer(pos, c);
                    }
        ...
        for (int i = 0; i < 3; i++)
        {
            aa << " " << i << ": " << program->GetOutputFixed(&i);
        }
       
        out.X = 15;
        out.Y = 23;

        g_Console.writeToBuffer(out, aa.str());
